...
public class TrailerReconnect : MonoBehaviour
{
    ...
    //         // === Break Forces ===
    //         joint.breakForce = 9000f;
    //         joint.breakTorque = 9000f;
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.CompareTag("Trailer"))
        {
            ...
            // Prevent multiple joints
            if (GetComponent<ConfigurableJoint>() != null)
                return;

            // Temporarily disable trailer physics to safely reposition
            trailerRb.isKinematic = true;

            // Optional: reference point on cart for hitch position
            // You can create a small empty GameObject on your cart called "HitchPoint"
            Transform hitchPoint = transform; // replace with hitchPoint transform if you have one

            // Snap trailer to hitch position without physics explosion
            trailerRb.transform.position = hitchPoint.position;
            trailerRb.transform.rotation = hitchPoint.rotation;

            // Re-enable physics
            trailerRb.isKinematic = false;
    ...
    void OnJointBreak(float breakForce)
    {
        ...
        Debug.Log($"Breakforce was: {breakForce}");
    }
